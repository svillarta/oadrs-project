...
class Auth extends CI_Controller
{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)){
				...
				foreach ($this->data['users'] as $k => $user){
					if($this->input->post('identity') == $user->email){
						$this->data['users'] = $this->ion_auth->get_users_groups($user->id)->result();
					}
				}
				...
				// echo json_encode($this->data['users'][0]);
				// redirect('/', 'refresh');
			}else{
	...
	public function create_user(){
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data,$this->input->post('group'))){
